...
expressApp.get("/api/board/:board_id", (req, res) => {
    ...
    database.findOne({ board_id }, (err, mongoResult) => {
        if (err) {
            res.sendStatus(404);
            return;
        }
...
expressApp.get("/api/cards/update", (req, res) => {
    ...
    database.findOne(query, (err, document) => {
        var list = document.lists.find(list => list.list_id === req.query.list_id);
        var card = list.cards.find(card => card.card_id === req.query.card_id);
...
expressApp.get("/api/boards/list", (req, res) => {
    var database = expressApp.get("database");
    database.find().toArray((err, mongoResult) => {
        if (err) {
            res.sendStatus(404);
            return;
        }
...
expressApp.get("/api/boards/rename", (req, res) => {
    var database = expressApp.get("database");
    var query = { board_id: req.query.board_id };
